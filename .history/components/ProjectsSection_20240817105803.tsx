      ...
      <div className="container bg-githubGray rounded-md mx-auto md:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-4 gap-8 p-4">
          {projects.map((project, index) => (
            <div key={index} className="bg-gptGray rounded-lg overflow-hidden shadow-deep-all transform transition-transform duration-300 hover:scale-110">
              <div className="relative w-full h-48 overflow-hidden">
                <div
                  className="absolute inset-0 flex infinite-scroll"
                  style={{ width: `${project.images.length * 100}%` }} // Dynamically set width
                >
                  {project.images.map((image, i) => (
                    <div key={i} className="flex-shrink-0 w-full h-full">
                      <img
                        ...
                        className="w-full h-full object-cover shadow-deep-all rounded-md"
                      />
                    </div>
                  ))}
                </div>
                <span className="shadow-deep-all absolute top-2 right-2 bg-green-500 text-xs text-white px-2 py-1 rounded-md">
                  {project.status}
                </span>
              </div>
              <div className="p-4">
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {project.technologies.map((tech, i) => (
                    <span key={i} className="bg-githubGray shadow-deep-all text-green-300 text-xs px-2 py-1 rounded-md">
                      {tech}
                    </span>
                  ))}
                </div>
